...
async fn live_fundamentals_smoke() {
    ...
    // earnings
    {
        let t = yfinance_rs::Ticker::new(client.clone(), "AMZN");
        let _ = t.earnings().await.unwrap();
    }
    // calendar
    {
        let t = yfinance_rs::Ticker::new(client.clone(), "META");
...
async fn live_fundamentals_for_record() {
    ...
    let t1 = yfinance_rs::Ticker::new(client.clone(), "AAPL");
    let _ = t1.quarterly_income_stmt().await;

    let t2 = yfinance_rs::Ticker::new(client.clone(), "MSFT");
    let _ = t2.balance_sheet().await;

    let t3 = yfinance_rs::Ticker::new(client.clone(), "GOOGL");
    let _ = t3.cashflow().await;

    let t4 = yfinance_rs::Ticker::new(client.clone(), "AMZN");
