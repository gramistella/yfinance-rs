...
fn extract_profile_fields(profile: &Profile) -> ProfileFields {
    match profile {
        Profile::Company(c) => ProfileFields {
            ...
            isin: c.isin.as_ref().map(|i| i.as_ref().to_string()),
            family: None,
            fund_kind: None,
        },
        Profile::Fund(f) => ProfileFields {
            ...
            summary: None,
            address: None,
            isin: f.isin.as_ref().map(|i| i.as_ref().to_string()),
...
fn assemble_info(
    ...
    Info {
        ...
        market_state: quote.and_then(|q| market_state_to_string(q.market_state)),
